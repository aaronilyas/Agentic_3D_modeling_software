"""MCP 2025-06-18 endpoint bound to one generic CAD application."""
    ...
    def recv_line(self, timeout: float = 5.0) -> str:
        ...
        while True:
            ...
            try:
                ...
                raise ConnectionError("MCP transport closed") from exc
            if not chunk:
...
class McpEndpoint:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._accept_loop, name="cad-mcp-accept", daemon=True)
        self._thread.start()
        ...
        while not self._closed and self._sock is not None:
            ...
            with self._clients_lock:
                if self._closed:
                    ...
                    return
                self._clients.append(transport)
            thread = threading.Thread(target=self._serve, args=(transport,), name="cad-mcp-session", daemon=True)
            thread.start()

    def _serve(self, transport: NdjsonSocket) -> None:
        try:
            try:
