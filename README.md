...
3.14. The installed distribution retains the name `agentic-jewelry-cad` and its
base dependencies for upgrade and jewelry compatibility. A coordinated package
rename/dependency split is deferred; both CLI entry points remain available.
...
with `STALE_SELECTION`. Queries are read-only: selections belong to callers,
not project state. IDs are scoped to the queried revision and body, not persistent
topological names across recomputation. `render_selection` highlights faces;
edge highlighting is explicitly unsupported and rejects nonempty `edge_ids`.
...
Cancellation leaves already committed operations undoable. This is the
`DeterministicPlanner` demo, not a general-purpose model agent. A provider-neutral
`Planner.decide(context)` interface can be injected with `Application(planner=...)`.
It receives document state, measurements, requested evidence, previous results,
and reference/render image resources with bytes separate from metadata.
...
Save does not modify the document. Open can be undone. Loading validates archive
paths and size limits, metadata, dependencies, and image checksums before commit.
Version-1 files remain supported; optional ID counters prevent reuse after open/undo.
...
`casting`. These are lightweight size/component heuristics, not complete process
simulation or a machinability guarantee. Jewelry wall and prong rules stay on the
jewelry validator.
...
With both compatible agent CLIs installed, `tests.run all` also exercises these
contracts against deterministic local inference. GUI assistant integration tests
likewise require those CLIs.
GitHub Actions runs the jewelry harness, fast, and integration layers, then
`tests.run generic`. A GUI job covers generic offscreen widgets and the jewelry
Qt/VTK suite under Xvfb.
...
Render tools return first-class MCP image blocks plus structured metadata;
`add_reference` and `inspect_reference` also return image content. Reference bytes
are available through MCP `resources/list` and `resources/read`. In-process
planners receive `ImageResource` objects. Direct Python render calls retain
`png_base64` for compatibility; MCP metadata uses `image_uri` instead.
